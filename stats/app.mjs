...
function lock(){token='';sessionStorage.removeItem(TOKEN_KEY);$('dashboard').hidden=true;$('lock').hidden=false;$('password').focus();}
...
function bars(rows,label){
 const max=Math.max(1,...rows.map(row=>Number(row.users)));
 return rows.length?rows.map(row=>{
  const percent=Math.max(2,Math.round(Number(row.users)/max*100));
  const level=Math.min(100,Math.max(10,Math.ceil(percent/10)*10));
...
if(token){$('lock').hidden=true;$('dashboard').hidden=false;refresh();}
