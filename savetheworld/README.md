...
The Vite base is `/savetheworld/`. The parent site's Netlify build places this app's output in `.netlify-publish/savetheworld/`. No client-side route rewrites are required: all screens use React state at the same URL. For GitHub Pages, copy `dist/` into the published site's `savetheworld/` directory. For Vercel hosting of the parent domain, publish this output at that same subdirectory. Refresh starts a new experiment.
