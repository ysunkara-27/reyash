import {progression,canPlayground} from './progression.mjs';
...
export async function readCare(env,userId,zone='UTC',now=new Date(),user=null){
 ...
 const dates=await env.DB.prepare('SELECT json_group_array(day) AS days FROM (SELECT DISTINCT day FROM dog_care_receipts WHERE user_id=?)').bind(userId).first();
 return {...careView(row,tasks,count.count,memory?{day:memory.day,action:memory.last_action,task:memory.last_task}:null),progress:progression(JSON.parse(dates.days||'[]'),day,count.count),playground:canPlayground(user)};
}
export async function handleCare(request,env,user,body,reply,now=new Date()){
 ...
 const care=await readCare(env,user.id,zone,now,user);
 ...
 return reply({care:await readCare(env,user.id,zone,now,user),action:action.id});
}
