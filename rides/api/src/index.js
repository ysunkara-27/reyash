...
export default {
  async fetch(request, env) {
    ...
    if (path === '/v1/login' && request.method === 'POST') {
      const { name, passcode } = await request.json(); if (!roster.some(r => r.name === name) || !validCode(passcode)) return json({ error: 'Invalid name or passcode.' }, 400, headers);
      const member = await env.DB.prepare('SELECT * FROM members WHERE name = ?').bind(name).first();
      if (!member) { if (name === 'Meera') return json({ error: 'The manager account must be set up first.' }, 403, headers); const salt = crypto.randomUUID(), codeHash = await hash(passcode, salt); await env.DB.prepare('INSERT INTO members (name, code_hash, salt) VALUES (?, ?, ?)').bind(name, codeHash, salt).run(); return json({ token: await tokenFor(name, 'rider', env.SESSION_SECRET), member: { name, role: 'rider' } }, 201, headers); }
